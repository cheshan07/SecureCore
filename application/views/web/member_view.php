    ...
    <div class="page-content">
        <div class="inner-box">
            <div class="dashboard-box">
                <h2 class="dashbord-title">Members</h2>
            </div>
            <?php
            if (isset($this->data['add_member'])) {
                ?>
...
<script>
    ...
    function getAllData() {
        $.ajax({
            type: 'post',
            url: '<?php echo base_url(); ?>members/get/all',
            data: {
    ...
    function getData(id) {
        $.ajax({
            type: 'post',
            url: '<?php echo base_url(); ?>members/get',
            data: {
                'id': id,
                <?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'
            },
    ...
    function saveData() {
        if (validate()) {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>members/save',
    ...
    function deleteData(id) {
        var bConfirm = confirm("Do you want to delete this member?");
        if (bConfirm) {
            $.ajax({
                type: 'post',
    ...
    function validate() {
        return (isNotEmpty("first_name", "First Name is required")
            && isSelected("gender", "Gender is required")
            && isNumeric("phone_no", "Phone Number is required")
            && isNotEmpty("nic", "NIC is required")
        );
    }
</script>